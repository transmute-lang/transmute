...
impl<'a> HtmlWriter<'a> {
    ...
    fn visit_struct(&mut self, stmt: StmtId, ident: IdentId, fields: &[Field]) {
        self.emit(XmlEvent::start_element("li").attr("id", &Self::type_id(stmt)));
        self.emit_keyword("struct");
        ...
        if !fields.is_empty() {
            ...
            for (i, field) in fields.iter().enumerate() {
                self.emit(XmlEvent::start_element("li"));
                self.emit_identifier(stmt, field.identifier().id(), Some(i));
                self.emit_colon();
                // fixme should be an emit_identifier_ref
                self.emit_identifier(stmt, field.ty().id(), Some(i));
                self.emit(XmlEvent::end_element())
            }
            self.emit(XmlEvent::end_element());
            self.emit(XmlEvent::end_element());
    ...
    fn visit_expression(&mut self, expr: ExprId) {
        match self.ast.expression(expr).kind() {
            ...
            ExpressionKind::Binary(left, op, right) => match op.kind() {
                BinaryOperatorKind::Access => self.visit_access(*left, *right),
                _ => unimplemented!("turned into functions during the resolution pass"),
            },
            ExpressionKind::Unary(_, _) => {
    ...
    fn visit_access(&mut self, left: ExprId, right: ExprId) {
        self.visit_expression(left);
        self.emit_dot();

        // todo link to struct field

        match self.ast.expression(right).kind() {
            ExpressionKind::Literal(lit) => match lit.kind() {
                LiteralKind::Identifier(ident_ref) => {
                    let ident_ref = self.ast.identifier_ref(*ident_ref);

                    // struct field are a special case in the sense that they are are not in the symbol table. (see
                    // Resolver::visit_struct()). This is why we have to implement the lookup login ain a custom way
                        ...
                        .map(|t| match t {
                            Type::Struct(stmt, fields) => {
                                let index = fields
                                    ...
                                    .enumerate()
                                    .find_map(
                                        |(i, (f, _))| if f == &ident { Some(i) } else { None },
                                    )
                                    .expect("field not found");
                                Self::ident_id(*stmt, Some(index))
                            }
                            Type::None => unimplemented!(),
                            _ => todo!(),
                        })
                        .expect("field not found (left type not found)");
                    ...
                    let ty = self
                        ...
                        .and_then(|t| match t {
                            ...
                            Type::Struct(stmt, _) => Some(Self::type_id(*stmt)),
                            Type::None => unimplemented!(),
                        })
    ...
    macro_rules! test_html_write {
        ($name:ident, $src:expr) => {
            ...
            fn $name() {
                ...
                let html = HtmlWriter::new(&ast, &symbols, &types).serialize();
